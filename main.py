...
from utils.helper_function import CutoutPIL
...
class CFG:
    ...
    save_dir = '/workspace/fashionai/save/'
    print_freq = 200
    num_workers = 1
    model_name = 'tf_efficientnet_b6'
    # tf_efficientnetv2_l_in21ft1k 384 512
    # dm_nfnet_f6 448 576 dm_nfnet_f4 384 512
    # resnext101_32x4d tresnet_xl_448 ecaresnet101d
    # tf_efficientnet_b6 now is the best!
    # tf_efficientnet_b6
    # vit_base_patch16_224_in21k need std to be [0.5, 0.5, 0,5] modify dataset config
    # img_tarin_size = [384, 384]
    # img_test_size = [512, 512]
...
def get_transforms(*, data):
    if data == 'train':
        return transforms.Compose([
            transforms.Resize((CFG.img_tarin_size[0], CFG.img_tarin_size[1])),
            CutoutPIL(cutout_factor=0.5),
            RandAugment(),
            transforms.ToTensor(),
            transforms.Normalize(mean=(0.649, 0.623, 0.611), std=(0.084, 0.088, 0.088)),
        ])
        ...
        #     A.Normalize(
        #         # lapel_design_labels
        #         mean=[0.634, 0.587, 0.568],
        #         std=[0.081, 0.085, 0.087],
        #     ),
        #     ToTensorV2()
        # ])
    elif data == 'test':
        return transforms.Compose([
            transforms.Resize((CFG.img_test_size[0], CFG.img_test_size[1])),
            transforms.ToTensor(),
            transforms.Normalize(mean=(0.649, 0.623, 0.611), std=(0.084, 0.088, 0.088)),
        ])
...
def train(train_loader, model, criterion, optimizer, epoch, device, scheduler, LOGGER):
    ...
    return acc
    # return acc, scores.avg


def validation(valid_loader, model, criterion, device, LOGGER):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    scores = AverageMeter()
    ...
    LOGGER.info('Test set: Average loss: {:.4f}, Accuracy: {} / {} ({:.3f}%)'.format(
...
def init_logger(log_dir):
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    log_file = CFG.log_dir + CFG.attribute + '_' + CFG.model_name + '_' + 'train.log'
...
def main():
    ...
    # o = model.forward_features(torch.randn(2, 3, 528, 528))
    # print(o.shape)

    save_folder = os.path.join(CFG.save_dir, CFG.attribute, CFG.model_name)
    print(f"model save to: {save_folder}")
    ...
    total_step = CFG.epochs * len(train_dataset)
    print(total_step)
    scheduler = CosineAnnealingWarmupRestarts(optimizer, first_cycle_steps=total_step, max_lr=CFG.lr, min_lr=CFG.min_lr,
                                              warmup_steps=1000)
    ...
    ####################### loss function ##########################################################
    # loss_fn = timm.loss.LabelSmoothingCrossEntropy()
    # new loss fun which is use onehot label  ce is normal label
    loss_fn = AsymmetricLossOptimized(gamma_neg=4, gamma_pos=1, clip=0.05)
    # loss_fn = AsymmetricLossSingleLabel(gamma_neg=2, gamma_pos=1, clip=0)
    ##############################################################################################

    best_score = 0
    for epoch in range(start_epoch + 1, CFG.epochs + 1):
        train_acc = train(train_loader, model, loss_fn, optimizer, epoch, device, scheduler, LOGGER=LOGGER)
        LOGGER.info('train set acc:{}%'.format(train_acc))

        score = validation(test_loader, model, loss_fn, device, LOGGER=LOGGER)
