...
class FashionAI(Dataset):
    ...
    base_folder = 'datasets/fashionAI'

    train_folder = 'fashionAI_attributes_train2'
    warm_folder = 'web'
    rank_folder = 'rank'
    ...
    def __init__(self, root, attribute, img_width, img_height, split=0.8, data_type='train', reset=False, ci=False,
        ...
        label_file = os.path.join(self.root, self.base_folder, self.train_folder, self.label_folder,
                                  self.train_label)
        # 找到特定的attribute标注  提供的labels是所有类别整合在一起的
        csvdata = []
        ...
        if self.data_type == 'train':
            ...
            for row in shuffle[:math.floor(self.split * len(shuffle))]:
                ...
                self.train_data.append(
                    np.uint8(np.array(self.loader(image_file).resize((self.width, self.height))).tolist()))
                self.train_labels.append(csvdata[row][2].find('y'))
                # csvdata[row][2].find('y') is a int 
                count += 1
    ...
    def download(self):
        ...
        for url, hash in zip(self.urls, self.hashs):
            ...
            with tarfile.open(file_path) as tar_f:
                ...
                safe_extract(tar_f, path=file_folder)
